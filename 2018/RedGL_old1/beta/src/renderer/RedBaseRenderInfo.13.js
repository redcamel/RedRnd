    ...
    RedBaseRenderInfo = function (redGL, redScene, callback) {
        ...
        this.render = function (time) {
            ...
            for (k in redGL['__datas']['RedProgramInfo']) {
                ...
                tProgramUniformLocationGroup = tempProgramInfo['uniforms']
                tGL.useProgram(tempProgramInfo['program'])
                // 퍼스팩티브 갱신
                tLocation = tProgramUniformLocationGroup['uPMatrix']['location']
                tGL.uniformMatrix4fv(tLocation, false, tScene['camera']['uPMatrix'])
                // 카메라갱신
                tLocation = tProgramUniformLocationGroup['uCameraMatrix']['location']
                tGL.uniformMatrix4fv(tLocation, false, tScene['camera']['uCameraMatrix'])
                if (tProgramUniformLocationGroup['uCameraPosition']) {
                    tLocation = tProgramUniformLocationGroup['uCameraPosition']['location']
                ...
                if (tProgramUniformLocationGroup['uSystemTime']) {
                    tLocation = tProgramUniformLocationGroup['uSystemTime']['location']
                    tGL.uniform1f(tLocation, time / 1000)
                }
                if (tProgramUniformLocationGroup['uSystemResolution']) {
                    tLocation = tProgramUniformLocationGroup['uSystemResolution']['location']
        ...
        this.setAmbientLight = (function () {
            ...
            return function (programInfo) {
                if (
                    ...
                    cacheColor[tLocationUUID = tLocation['__UUID']] == tColorList ? 0 : tGL.uniform4fv(tLocation, tColorList)
                    cacheColor[tLocationUUID] = tColorList
                }
            }
        })()
        // 디렉셔널 라이트 유니폼세팅
        this.setDirectionalLight = (function () {
            var tDirectionList = [],
                tDirectionColorList = []
            var tLocation,tLocationUUID
            var cacheDirectionColor = []
            var cacheDirection = []
            var cacheDirectionNum = []
            var tDirectionNum 
            var tProgramUniformLocationGroup
            return function (programInfo) {
                ...
                if (
                    ...
                    tProgramUniformLocationGroup['uDirectionnalLightDirection']
                ) {
                    tDirectionList.length = 0
                    tDirectionColorList.length = 0
                    tScene['lights'][RedDirectionalLightInfo.TYPE].forEach(function (v, index) {
                        ...
                        tDirectionColorList[index * 4 + 0] = v['color'][0]
                        tDirectionColorList[index * 4 + 1] = v['color'][1]
                        tDirectionColorList[index * 4 + 2] = v['color'][2]
                        tDirectionColorList[index * 4 + 3] = v['color'][3]
                    })
                    // 방향
                    tLocation = tProgramUniformLocationGroup['uDirectionnalLightDirection']['location']
                    cacheDirection[tLocationUUID = tLocation['__UUID']] == tDirectionList ? 0 : tGL.uniform3fv(tLocation, new Float32Array(tDirectionList))
                    cacheDirection[tLocationUUID] = tDirectionList

                    // 컬러 
                    tLocation = tProgramUniformLocationGroup['uDirectionnalLightColor']['location']
                    cacheDirectionColor[tLocationUUID = tLocation['__UUID']] == tDirectionColorList ? 0 : tGL.uniform4fv(tLocation, new Float32Array(tDirectionColorList))
                    cacheDirectionColor[tLocationUUID] = tDirectionColorList
                    // 갯수
                    tLocation = tProgramUniformLocationGroup['uDirectionalNum']['location']
                    tDirectionNum = tScene['lights'][RedDirectionalLightInfo.TYPE].length
                    cacheDirectionNum[tLocationUUID = tLocation['__UUID']] == tDirectionNum ? 0 : tGL.uniform1i(tLocation, tDirectionNum)
                    cacheDirectionNum[tLocationUUID] = tDirectionNum
                }
            }
        })()
        // 포인트 라이트 유니폼 세팅
        this.setPointLight = (function () {
            var tPointList = [],
                tPointColorList = [],
                tPointRadius = new Float32Array(16)
            var tLocation,tLocationUUID
            var tProgramUniformLocationGroup
            var cachePointPosition = []
            var cachePointColor = []
            var cachePoint = []
            var cachePointRadius = []
            var cachePointNum = []
            var tPointNum 
            return function (programInfo) {
                tProgramUniformLocationGroup = programInfo['uniforms']
                if (
                    ...
                    tProgramUniformLocationGroup['uPointNum']
                ) {
                    tPointList.length = 0
                    tPointColorList.length = 0
                    ...
                    tScene['lights'][RedPointLightInfo.TYPE].forEach(function (v, index) {
                        ...
                        tPointColorList[index * 4 + 0] = v['color'][0]
                        tPointColorList[index * 4 + 1] = v['color'][1]
                        tPointColorList[index * 4 + 2] = v['color'][2]
                        tPointColorList[index * 4 + 3] = v['color'][3]
                    ...
                    // 포지션
                    tLocation = tProgramUniformLocationGroup['uPointLightPosition']['location']
                    cachePointPosition[tLocationUUID = tLocation['__UUID']] == tPointList ? 0 : tGL.uniform3fv(tLocation, new Float32Array(tPointList))
                    cachePointPosition[tLocationUUID] = tPointList
                    // 컬러
                    tLocation = tProgramUniformLocationGroup['uPointLightColor']['location']
                    cachePointColor[tLocationUUID = tLocation['__UUID']] == tPointColorList ? 0 : tGL.uniform4fv(tLocation, new Float32Array(tPointColorList))
                    cachePointColor[tLocationUUID] = tPointColorList
                    // 갯수
                    tLocation = tProgramUniformLocationGroup['uPointNum']['location']
                    tPointNum = tScene['lights'][RedPointLightInfo.TYPE].length
                    cachePointNum[tLocationUUID = tLocation['__UUID']] == tPointNum ? 0 : tGL.uniform1i(tLocation, tPointNum)
                    cachePointNum[tLocationUUID] = tPointNum
                    // 반경
                    tLocation = tProgramUniformLocationGroup['uPointLightRadius']['location']
                    cachePointRadius[tLocationUUID = tLocation['__UUID']] == tPointRadius ? 0 : tGL.uniform1fv(tLocation, tPointRadius)
                    cachePointRadius[tLocationUUID] = tPointRadius
                }
            }
        })()
        // 바닥그리드 draw
        ...
        this.draw = function (renderList, time, parentMTX) {
            ...
            while (i--) {
                ...
                while (i2--) {
                        ...
                        if (tUniformValue && tUniformValue['__webglTextureYn']) {
                            ...
                            if (tUniformValue['loaded']) {
                                ...
                                if (tUseMapKey = tUSE_MAP[tUniformKey]) {
                                    if (tTargetIndex != RedTextureIndex[tUSE_MAP[tUniformKey][2]]) {
                                        console.log(tTexture)
                                        console.log(tUseMapKey, tUniformKey, tTargetIndex, RedTextureIndex[tUSE_MAP[tUniformKey][2]])
                                        throw tUSE_MAP[tUniformKey][2] + " 인덱스타입이 아닙니다."
                                    }
                                    tCacheUseTexture[tLocation.__UUID] == 1 ? 0 : targetGL.uniform1i(
                                        tProgramUniformLocationGroup[tUseMapKey = tUseMapKey[0]]['location'],
                                        tCacheUseTexture[tLocation.__UUID] = 1
                                    )
                                }
                            } else tRenderable = false
                        } else if (tUseMapKey = tUSE_MAP[tUniformKey]) {
                            tCacheUseTexture[tLocation.__UUID] == 0 ? 0 : targetGL.uniform1i(
                                tProgramUniformLocationGroup[tUseMapKey = tUseMapKey[0]]['location'],
                    ...
                    else if (tRenderType == RedConst.INT) tCacheIntFloat[tUniformKey] == tUniformValue ? 0 : targetGL.uniform1i(tLocation, tCacheIntFloat[tUniformKey] = tUniformValue)
                    else if (tRenderType == RedConst.FLOAT) tCacheIntFloat[tUniformKey] == tUniformValue ? 0 : targetGL.uniform1f(tLocation, tCacheIntFloat[tUniformKey] = tUniformValue)
