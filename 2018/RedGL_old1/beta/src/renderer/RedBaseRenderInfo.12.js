                    ...
                    if (tRenderType == RedConst.SAMPLER) {
                        if (tUniformValue && tUniformValue['__webglTextureYn']) {
                            ...
                            if (tUniformValue['loaded']) {
                                ...
                                if (tCacheTexture[targetIndex] != tUniformValue['__UUID']) {
                                    targetGL.activeTexture(GL_TEXTURE0 + targetIndex),
                                        ...
                                        targetGL.uniform1i(tLocation, targetIndex),
                                        tCacheTexture[targetIndex] = tUniformValue['__UUID']
                                }
                                // 사용여부를 결정
                                if (tUseMapKey = tUSE_MAP[tUniformKey]) {
                                    ...
                                    if (targetIndex != RedTextureIndex[tUSE_MAP[tUniformKey][2]]) {
                                        console.log(tTexture)
                                        console.log(tUseMapKey, tUseMapTextureKey, targetIndex, RedTextureIndex[tUSE_MAP[tUniformKey][2]])
                                        throw tUSE_MAP[tUniformKey][2] + " 인덱스타입이 아닙니다."
                                    }
                                    tCacheUseTexture[tLocation.__UUID] == 1 ? 0 : targetGL.uniform1i(tProgramUseUniformValue.location, tCacheUseTexture[tLocation.__UUID] = 1)
                                }
                            ...
                            tUseMapKey = tUseMapKey[0],
                                tUseMapTextureKey = tUniformKey,
                                tProgramUseUniformValue = tProgramUniformLocationGroup[tUseMapKey],
                                tCacheUseTexture[tLocation.__UUID] == 0 ? 0 : targetGL.uniform1i(tProgramUseUniformValue.location, tCacheUseTexture[tLocation.__UUID] = 0)
                        }
