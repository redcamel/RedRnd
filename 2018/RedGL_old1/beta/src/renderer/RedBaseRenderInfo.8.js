...
(function () {
    ...
    RedBaseRenderInfo = function (redGL, redScene, callback) {
        ...
        this.draw = function (renderList, time, parentMTX) {
            ...
            var parentClone;
            var tempData
            var tRotation, tPosition, tScale
            ...
            while (i--) {
                ...
                while (i2--) {
                    ...
                    if (tRenderType == INT) cacheInt == tUniformValue ? 0 : targetGL.uniform1i(tLocation, tUniformValue), cacheInt = tUniformValue
                    else if (tRenderType == FLOAT) cacheFloat == tUniformValue ? 0 : targetGL.uniform1f(tLocation, tUniformValue), cacheFloat = tUniformValue
                        ...
                        if (tUniformValue && tUniformValue.__webglTextureYn) {
                            ...
                            if (tUniformValue.loaded) {
                                ...
                                if (cacheTexture_UUID[targetIndex] != tUniformValue.__UUID) {
                                    ...
                                    cacheTexture_UUID[targetIndex] == tUniformValue.__UUID ? 0 : targetGL.uniform1i(tLocation, targetIndex)
                                ...
                                if (tUseMapKey) {
                                    ...
                                    cacheUseTexture[tLocation.__UUID] == 1 ? 0 : targetGL.uniform1i(tProgramUseUniformValue.location, 1)
                                    cacheUseTexture[tLocation.__UUID] = 1
                            ...
                            if (tUseMapKey) {
                                ...
                                tUseMapTextureKey = tUniformKey
                                tProgramUseUniformValue = tProgramUniformLocationGroup[tUseMapKey]
                                cacheUseTexture[tLocation.__UUID] == 0 ? 0 : targetGL.uniform1i(tProgramUseUniformValue.location, 0)
                                cacheUseTexture[tLocation.__UUID] = 0
                            }
                        }

                    }
                    else if (tRenderType == MAT) targetGL[tUniformValue.__uniformMethod](tLocation, false, tUniformValue)
                    else if (tRenderType == VEC) targetGL[tUniformValue.__uniformMethod](tLocation, tUniformValue)
                    // 아틀라스코디네이트값인경우
                    else if (tRenderType == ATLASCOORD) cacheUVAtlascoord_UUID == tUniformValue.__UUID ? 0 : targetGL.uniform4fv(tLocation, tUniformValue.value), cacheUVAtlascoord_UUID = tUniformValue.__UUID
                    // 이도저도아닌경우는 뭔가 잘못된거임
                ...
                targetGL.uniformMatrix4fv(tProgramUniformLocationGroup.uMVMatrix.location, false, tMVMatrix),
                    ...
                    // 컬페이스 사용여부 캐싱처리
                    cacheUseCullFace != tMesh.useCullFace ? (cacheUseCullFace = tMesh.useCullFace) ? targetGL.enable(GL_CULL_FACE) : targetGL.disable(GL_CULL_FACE) : 0,
                    cacheCullFace != tMesh.cullFace ? targetGL.cullFace(tMesh.cullFace) : 0, cacheCullFace = tMesh.cullFace,
                    // 뎁스테스트 사용여부 캐싱처리
                    cacheUseDepthTest != tMesh.useDepthTest ? (cacheUseDepthTest = tMesh.useDepthTest) ? targetGL.enable(GL_DEPTH_TEST) : targetGL.disable(GL_DEPTH_TEST) : 0,
                    // 뎁스테스팅 캐싱처리
                ...
                if (tIndicesBuffer) {
                    bitmapRenderable ?
                        ...
                        : 0
                } else targetGL.drawArrays(tMesh.drawMode, 0, tVertexPositionBuffer.pointNum), cacheDrawBufferUUID = tVertexPositionBuffer.__UUID
                // 자식을 콜
