    ...
    RedBaseRenderInfo = function (redGL, redScene, callback) {
        ...
        this.render = function (time) {
            ...
            for (k in redGL['__datas']['RedProgramInfo']) {
                ...
                tScene['lights'][RedDirectionalLightInfo.TYPE].length ? self.setDirectionalLight(tempProgramInfo) : 0,
                tScene['lights'][RedPointLightInfo.TYPE].length ? self.setPointLight(tempProgramInfo) : 0,
                // 시스템 타임, 레졸루션 갱신
            ...
            while (i--) {
                tSelf['numDrawCall']++ ,
                    ...
                    cacheProgram != tProgram ? (
                        ...
                        // cacheUVAtlascoord_UUID.length = 0
                    ) : 0,
                    ...
                    tCacheUVAtlascoord_UUID = cacheUVAtlascoord_UUID
                    cacheProgram = tProgram,
                ...
                while (i2--) {
                    ...
                    else if (tRenderType == RedConst.INT) tCacheIntFloat[tLocationUUID] == tUniformValue ? 0 : targetGL.uniform1i(tLocation, tCacheIntFloat[tLocationUUID] = tUniformValue)
                    else if (tRenderType == RedConst.FLOAT) tCacheIntFloat[tLocationUUID] == tUniformValue ? 0 : targetGL.uniform1f(tLocation, tCacheIntFloat[tLocationUUID] = tUniformValue)
                    ...
                    // 아틀라스코디네이트값인경우
                    else if (tRenderType == RedConst.ATLASCOORD) tCacheUVAtlascoord_UUID[tLocationUUID] == tUniformValue['__UUID'] ? 0 : targetGL.uniform4fv(tLocation, tUniformValue['value']), tCacheUVAtlascoord_UUID[tLocationUUID] = tUniformValue['__UUID']
                    // 이도저도아닌경우는 뭔가 잘못된거임
