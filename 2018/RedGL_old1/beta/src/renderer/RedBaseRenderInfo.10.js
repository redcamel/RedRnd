            ...
            while (i--) {
                ...
                while (i2--) {
                    ...
                    if (tAttrLocationGroup[tAttrPointerKey]) {
                        ...
                        tCacheAttrUUID[tLocation] == tAttrBufferInfo['__UUID'] ?
                            ...
                            (
                                // 실제 버퍼 바인딩하고
                                targetGL.bindBuffer(targetGL.ARRAY_BUFFER, tAttrBufferInfo['buffer']),
                        ...
                        if (tUniformValue && tUniformValue['__webglTextureYn']) {
                            ...
                            if (tUniformValue['loaded']) {
                                ...
                                if (tCacheTexture_UUID[targetIndex] != tUniformValue['__UUID']) {
                                    targetGL.activeTexture(targetGL.TEXTURE0 + targetIndex),
                                    targetGL.bindTexture(tUniformValue['__webglTexture'] ? targetGL.TEXTURE_2D : targetGL.TEXTURE_CUBE_MAP, tUniformValue['texture']),
                ...
                if (!emptyCubeMap) {
                    if (tProgramUniformLocationGroup['uUseReflectionTexture'] || tProgramUniformLocationGroup['uUseRefractionTexture']) {
                        ...
                        if (tProgramUniformLocationGroup['uUseReflectionTexture']) {
                            targetGL.activeTexture(targetGL.TEXTURE0 + RedTextureIndex.CUBE_REFLECTION)
                            targetGL.bindTexture(targetGL.TEXTURE_CUBE_MAP, emptyCubeMap['texture'])
                        ...
                        if (tProgramUniformLocationGroup['uReflectionTexture']) {
                            targetGL.activeTexture(targetGL.TEXTURE0 + RedTextureIndex.CUBE_REFRACTION)
                            targetGL.bindTexture(targetGL.TEXTURE_CUBE_MAP, emptyCubeMap['texture'])
                ...
                if (tIndicesBuffer) {
                    if (tBitmapRenderable) {
                        cacheDrawBufferUUID == tIndicesBuffer['__UUID'] ? 0 : targetGL.bindBuffer(targetGL.ELEMENT_ARRAY_BUFFER, tIndicesBuffer['buffer'])
                        targetGL.drawElements(tMesh['drawMode'], tIndicesBuffer['pointNum'], targetGL.UNSIGNED_SHORT, 0)
